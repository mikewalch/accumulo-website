...
| plink | Creates Property link  | Assumes server property by default. Use `-c` to link to client properties. Can link to property prefixes with `.*` (ex. `table.*`)  | `{% plink -c instance.name %}`                             |
| purl  | Creates Property URL   | Default is servery property. Use `-c` to link to client properties. Can link to property prefixes with `.*` (ex. `table.*`)              | `{% purl instance.volumes %}`                             |
